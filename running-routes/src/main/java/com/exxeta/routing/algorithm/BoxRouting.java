...
public class BoxRouting implements RoutingAlgorithm {

    private static Logger logger = Logger.getLogger(BoxRouting.class.getName());
    ...
    public double doRouting(Node startNode, Graph g, List<Point> collection, double distance) {
        ...
        while (currentDistance < distance / 4) {
        ...
        // calculate one quarter
        while (currentDistance < distance * 3 / 4) {
            randomEdge = getBestEdge(currentNode, angle);
        ...
        DijkstraShortestPathFinder pf = new DijkstraShortestPathFinder(g, startNode, weighter);
        pf.calculate();
